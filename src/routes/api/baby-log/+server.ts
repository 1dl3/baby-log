...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    const addTypeInfo = (entries, type) => entries.map(entry => ({ ...entry, logType: type }));
    ...
    if (!type || type === 'all') {
      ...
      const allEntries = [
        ...
        ...addTypeInfo(sleepEntries.map(entry => ({
          ...entry,
          timestamp: entry.startTime // Normalize timestamp field
        })), 'sleep'),
        ...addTypeInfo(medicationEntries.map(entry => ({
          ...entry,
          timestamp: entry.administeredAt // Normalize timestamp field
        })), 'medication'),
        ...addTypeInfo(milestoneEntries.map(entry => ({
          ...entry,
      ...
      if (type === 'diaper') {
        ...
        countResult = await db.select({ count: db.fn.count() }).from(diaperChange).where(
          and(
            eq(diaperChange.babyId, babyId),
            ...(start ? [gte(diaperChange.timestamp, start)] : []),
            ...(end ? [lte(diaperChange.timestamp, end)] : [])
        ...
        countResult = await db.select({ count: db.fn.count() }).from(feeding).where(
          and(
            eq(feeding.babyId, babyId),
            ...(start ? [gte(feeding.timestamp, start)] : []),
            ...(end ? [lte(feeding.timestamp, end)] : [])
        ...
        countResult = await db.select({ count: db.fn.count() }).from(nursing).where(
          and(
            eq(nursing.babyId, babyId),
            ...(start ? [gte(nursing.timestamp, start)] : []),
            ...(end ? [lte(nursing.timestamp, end)] : [])
        ...
        countResult = await db.select({ count: db.fn.count() }).from(sleep).where(
          and(
            eq(sleep.babyId, babyId),
            ...(start ? [gte(sleep.startTime, start)] : []),
            ...(end ? [lte(sleep.startTime, end)] : [])
        ...
        totalCount = countResult[0].count;
        logEntries = addTypeInfo(logEntries.map(entry => ({
          ...entry,
          timestamp: entry.startTime // Normalize timestamp field
        })), 'sleep');
        ...
        countResult = await db.select({ count: db.fn.count() }).from(medication).where(
          and(
            eq(medication.babyId, babyId),
            ...(start ? [gte(medication.administeredAt, start)] : []),
            ...(end ? [lte(medication.administeredAt, end)] : [])
        ...
        countResult = await db.select({ count: db.fn.count() }).from(milestone).where(
          and(
            eq(milestone.babyId, babyId),
            ...(start ? [gte(milestone.achievedAt, start)] : []),
            ...(end ? [lte(milestone.achievedAt, end)] : [])
        ...
        countResult = await db.select({ count: db.fn.count() }).from(measurement).where(
          and(
            eq(measurement.babyId, babyId),
            ...(start ? [gte(measurement.timestamp, start)] : []),
            ...(end ? [lte(measurement.timestamp, end)] : [])
        ...
        countResult = await db.select({ count: db.fn.count() }).from(photo).where(
          and(
            eq(photo.babyId, babyId),
            ...(start ? [gte(photo.timestamp, start)] : []),
            ...(end ? [lte(photo.timestamp, end)] : [])
